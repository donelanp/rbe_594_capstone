function [m_anom] = magnetic(x, y, origin, region)
% Compute magnetic anomaly from World Digital Magnetic Anomaly Map with
...
%
% Outputs:
%   m_anom - magnetic anomaly (1 x n) [nT]

persistent Fm cached_file

lat0       = origin(1);
lon0       = origin(2);
cache_name = sprintf('magnetic_lat%0.2f_lon%0.2f.mat', lat0, lon0);
...
if isempty(cached_file) || ~strcmp(cached_file, cache_name)
    if isfile(cache_name)
        fprintf('Loading cached magnetic anomaly data from %s ...\n', cache_name);
        data = load(cache_name);
    else
        fprintf('Downloading magnetic anomaly data for origin [%0.2f, %0.2f] ...\n', lat0, lon0);
        data = download_magnetic_data(origin, region);
        save(cache_name, '-struct', 'data', '-v7.3');
        fprintf('Saved %s\n', cache_name);
    end

    Fm          = griddedInterpolant({data.lon, data.lat}, data.z, 'linear', 'nearest');
...
function [data] = download_magnetic_data(origin, region)
lat0 = origin(1);
lon0 = origin(2);
...
nc_file = [tempname() '.nc'];
gmt_cmd = sprintf('gmt grdcut @earth_wdmam_03m -R%.6f/%.6f/%.6f/%.6f -G%s', ...
