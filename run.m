...
% z = [x (3*(N+1)); u (2*(N+1)); tauf (1)]
constraints = @(z) nonlcon(z, x0, xf, current_field, wheelbase, max_speed, max_steer, N);
...
[~, x_s] = lpm.simulate(x0, u, tauf, @(tau, x, u) system_dynamics(tau, x, u, current_field, wheelbase, max_speed, max_steer));
...
%% compute constraints
function [c, ceq] = nonlcon(z, x0, xf, current_field, wheelbase, max_speed, max_steer, N)
...
function [xdot] = system_dynamics(~, x, u, current_field, wheelbase, max_speed, max_steer)
theta = x(3,:);
v     = u(1,:) * max_speed;
delta = u(2,:) * max_steer;
